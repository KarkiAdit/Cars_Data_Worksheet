 ...
 * import express */
 ...
 app.put("/edit", (request, response) => {
   const carId = request.body.id;
   let returnResponse = {};
   const carToEdit = carsMockData.filter(
     (cardData) => cardData.id === carId
   );
 ...
 app.delete("/delete", (request, response) => {
   const carId = request.body.id;
   let retResponse = {};
   const initialLength = carsMockData.length;
 
   //   Remove the car from carsMockData if the id matches
   carsMockData = carsMockData.filter((carData) => carData.id !== carId);
 
   if (carsMockData.length === initialLength) {
     // check if item was deleted or not
     retResponse = { status: 500, message: "No car with this id exists" };
   } else {
     retResponse = {
       status: 200,
